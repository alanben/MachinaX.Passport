...
/*	----------------------------------------------------------------------------------------------------------------------------------------------	
	...
	20080102:	Removed old login procedures ([x_UserLoginOld] and [x_UserLoginAlt])
	20080103:	Added procedures:
				- x_UserSetLoginFail
				- x_UserSetLoginOK
	----------------------------------------------------------------------------------------------------------------------------------------------	*/
...
/*	-----------------------------------------------------------------------
	...
	if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[x_UserLoginGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	drop procedure [dbo].[x_UserLoginGet]
	GO
...
/*	-----------------------------------------------------------------------
	...
	SET ANSI_NULLS ON 
	GO
	CREATE Procedure x_UserLoginGet (
		@UserName varchar(50)
	) 
	As
		Begin
...
/*	-----------------------------------------------------------------------
	...
	Parameters:
		@UserID:		User identifier
		@FailCount:		The count of login failures
		@FailDate:		The date of the last login failure
		@LockDate:		The date for the expiry of the login lock on the account
	Returns:
		void
	----------------------------------------------------------------------- */
	SET QUOTED_IDENTIFIER ON 
	...
	SET ANSI_NULLS ON 
	GO
	CREATE Procedure x_UserSetLoginFail (
		@UserID int,
		@FailCount int,
		@FailDate datetime,
		@LockDate datetime
...
/*	-----------------------------------------------------------------------
	...
	SET ANSI_NULLS ON 
	GO
	CREATE Procedure x_UserSetLoginOK (
		@UserID int,
		@FailCount int,
		@Token uniqueidentifier,
		@LoginDate datetime,
